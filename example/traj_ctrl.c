...
void cst_test(MotorData_t *m_data_d, MotorData_t *m_data_d2, double dt)
{
  static double time = 0;
  WaveParam_t wp;
  wp.A = 2;
  ...
  for (uint16_t i = 0; i < motor_cfg.num; i++)
  {
    m_data_d2[i] = m_data_d[i];
    m_data_d[i].tau = square_wave(&wp, time);
    m_data_d[i].tau_ff = 0;
    m_data_d[i].max_tau = MAX_CURRENT;
  }
  em_set_torques(ids, motor_cfg.num, m_data_d);
...
void csv_test(MotorData_t *m_data_d, MotorData_t *m_data_d2, double dt)
{
  static double time = 0;
  WaveParam_t wp;
  wp.A = 180;
  ...
  em_set_velocities(ids, motor_cfg.num, m_data_d);
...
void csp_test(MotorData_t *m_data_d, MotorData_t *m_data_d2, double dt)
{
  static double time = 0;
  WaveParam_t wp;
  wp.A = 90;
  ...
  em_set_positions(ids, motor_cfg.num, m_data_d);
...
OSAL_THREAD_FUNC_RT control_thread(void *ptr)
{
  ...
  while (th_run)
  {
    ...
    csp_test(m_data_d, m_data_d2, dt);
...
int main(int argc, char *argv[])
{
  ...
  go_pos(ids, motor_cfg.num, initial_pos, 300, dt);
